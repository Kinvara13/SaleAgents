<template>
  <div class="fade-in flex flex-col" style="height: calc(100vh - 48px)">
    ...
    <!-- 回标文件编写 Tab 内容 -->
    <div v-if="activeTab === 'writing'" class="flex-1 flex flex-col min-h-0 overflow-auto">
      <!-- 文件目录 -->
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 mb-3 overflow-hidden flex-shrink-0">
        ...
        <div v-if="!isLeftSidebarHidden" class="p-3 space-y-3 max-h-[240px] overflow-y-auto">
          ...
          <div v-else-if="fileTree.length === 0" class="text-sm text-gray-400 text-center py-4">
            {{ selectedProjectId ? '暂无回标模板，请先上传回标文件模板' : '请选择一个项目' }}
          </div>
          <div
      ...
      <!-- 下方三栏布局 -->
      <div class="flex-1 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex min-h-[180px]">
        <!-- 左侧：AI生成后文件预览 -->
        <div :class="[
          'border-r border-gray-100 flex flex-col transition-all duration-300 h-full overflow-hidden min-w-0',
          isRightTemplateHidden ? 'w-full' : 'w-5/12'
        ]">
        ...
        <div
          :class="[
            'border-r border-gray-100 flex flex-col transition-all duration-300 h-full overflow-hidden min-w-0',
            isRightTemplateHidden ? 'w-0 hidden' : 'w-4/12'
          ]"
        >
          <div class="flex items-center justify-between p-4 border-b border-gray-100 bg-gray-50">
            <div class="flex items-center space-x-3">
              ...
              <div class="bg-gray-50 rounded-lg p-4">
                ...
                <div class="text-sm text-gray-600 space-y-2">
                  ...
                  <div class="mt-4 border-t border-gray-200 pt-4">
                    <p class="text-gray-500 text-xs mb-2">这是回标模板的原始内容：</p>
        ...
        <!-- 右侧：AI对话框 -->
        <div :class="[
          'flex flex-col transition-all duration-300 h-full overflow-hidden min-w-0',
          isRightTemplateHidden ? 'w-1/3' : 'w-3/12'
        ]">
          <!-- 选择元素按钮 -->
          ...
          >
            ...
            <template #input-suffix>
              <div class="flex items-center justify-end px-3 py-2 border-t border-gray-100 min-w-0">
                <select
                  v-model="selectedAIModel"
                  class="w-full max-w-full px-2 py-1.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary text-sm text-gray-700 truncate"
                >
...
const expandedSections = ref<Set<string>>(new Set(['business', 'technical', 'proposal', 'other']))
...
const parsingSections = ref<TenderSection[]>([])
const templateFiles = ref<FileItem[]>([])
...
const selectProject = (project: Project) => {
  const fieldMap = getExtractedFieldMap(project)
  const displayName = fieldMap['项目名称'] || project.name || '-'
...
async function loadProjects() {
  ...
  try {
    ...
    const projectIdFromRoute = String(route.query.projectId || '')
    if (projectIdFromRoute && !selectedProjectId.value) {
      const matched = projects.value.find(project => project.id === projectIdFromRoute)
      if (matched) {
...
async function loadSections(projectId: string) {
  ...
  try {
    ...
    if (project && project.bid_template_files && Array.isArray(project.bid_template_files) && project.bid_template_files.length > 0) {
      templateFiles.value = project.bid_template_files
        .filter((f: any) => f?.selected !== false)
        .map((f: any) => ({
        id: f.id,
        name: f.name,
        completed: false,
        path: f.path || f.name,
        sectionType: f.section_type || 'other',
      }))
    }
  } catch (e: any) {
...
const fileTree = computed((): FileSection[] => {
  ...
  for (const file of templateFiles.value) {
    const type = file.sectionType || 'other'
    groups[type] = groups[type] || []
    groups[type].push(file)
  }

  const typeMeta: Record<string, { name: string; icon: string }> = {
    'business': { name: '商务部分', icon: '📁' },
    'technical': { name: '技术部分', icon: '📄' },
    'proposal': { name: '方案/报价部分', icon: '📝' },
    'other': { name: '其他回标模板', icon: '📋' },
  }
...
const handleSelectFile = async (fileId: string) => {
  ...
  const targetFile = templateFiles.value.find(file => file.id === fileId)
  if (!targetFile?.path) return
  loadingDetail.value = true
  try {
    const preview = await previewBidTemplateFile(selectedProjectId.value, targetFile.path)
    sectionDetail.value = {
      section_name: targetFile.name,
      section_type: targetFile.sectionType || 'other',
      source_file: preview.filename || targetFile.path,
      content: preview.content || '',
    }
    // BUG-006 修复：编辑模式下切换文件时同步更新编辑内容
    if (isEditing.value) {
      editableContent.value = preview.content || ''
    }
  } catch (e: any) {
...
const saveSection = async () => {
  if (!selectedProjectId.value || !selectedFile.value || !sectionDetail.value) return
  saving.value = true
  try {
    sectionDetail.value = {
      ...sectionDetail.value,
